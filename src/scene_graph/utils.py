...
def get_text_description(file_path):
    with open(file_path, 'r') as f:
        data = json.load(f)
        user_input = data['description']
    return user_input

def count_interior_items(file_path):
    with open(file_path, 'r') as f:
        data = json.load(f)
        count = data['no_of_objects']
    return count
...
def get_rotation(obj_A, scene_graph):
    ...
    if obj_A is not None:
        ...
        if obj_A["new_object_id"] in layout_rot.keys():
...
def check_wall_relationship_impossibilities(G, scene_graph):
    ...
    for node in topological_order:
        if node not in ROOM_LAYOUT_ELEMENTS:
            parents_raw = list(G.predecessors(node))
            parents = list(filter(lambda x : x not in ROOM_LAYOUT_ELEMENTS, parents_raw))
            #parents_rot = [get_rotation(next((p for p in parents), None), scene_graph)]
            parents_rot = [get_rotation(next((x for x in scene_graph if x["new_object_id"] == p), None), scene_graph) for p in parents]
            # Check whether the parent object is in the corner and if this object is located spatially correctly
            for p, r in zip(parents, parents_rot): 
...
def create_empty_image_with_boxes(image_size, boxes):
    ...
    #img = cv2.imread('23. Windows Logo 2.jpg')
    for box in boxes:
        x, y, w, h, r, label = box
...
def place_object(obj, scene_graph, room_dimensions, errors={}, verbose=True):
    ...
    while True:
        ...
        for child in children:
            ...
            errors_child = place_object(child, scene_graph, room_dimensions, errors={})# не это место
            if verbose:
